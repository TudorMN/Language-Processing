...
public class AnalizadorLexico {
    ...
    private ArrayList<String> opmul;
    ...
    public int delta (int estado, char c) {
        switch (estado) {
            case 1:
                ...
                if (c == ',')
                    return returnLexeme(Token.COMA, c); //coma
                if (c == ':')
                    return returnState(8, c);
                if (c == '[')
                    return returnLexeme(Token.CORI, c); //cori
                if (c == ']')
                    return returnLexeme(Token.CORD, c); //cord
                if (c == ';')
                ...
                if (c == '*' || c == '/')
                    return returnLexeme(Token.OPMUL, c); //opmul
                if ((c >= 'a' && c <= 'z') || (c >= 'A' && c <= 'Z'))
            ...
            case 14:
                if (c == '.') {
                    return returnLexeme(Token.PTOPTO, c); //ptopto
                }
                else{
                    //o_col++;
                    //col++;
